...
class Flying_eye(pygame.sprite.Sprite):
    def __init__(self, pos, player, obstacle_sprites):
        ...
        self.projectile.status = 'explode'
    ...
    def get_status(self):
        if sqrt((self.coordx - self.player.coordx) ** 2 + (self.coordy - self.player.coordy) ** 2) < 160:
            self.create_projectile([self.coordx, self.coordy], [self.player.coordx, self.player.coordy])
...
class Goblin(Flying_eye):
    def __init__(self, pos, player, obstacle_sprites):
        ...
        self.projectile = Goblin_projectile(pos=pos, player=self.player)
